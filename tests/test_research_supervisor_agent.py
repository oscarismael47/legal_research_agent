...
from agents.deep_research.research_supervisor_agent import build_graph
...
if __name__ == "__main__":
    ...
    research_supervisor_agent = build_graph(checkpointer=checkpointer)

    graph_image = research_supervisor_agent.get_graph(xray=True).draw_mermaid_png()

    ## save graph image for visualization
    with open("./static_images/research_supervisor_agent_graph.png", "wb") as f:
        f.write(graph_image)
    ...
    result = research_supervisor_agent.invoke({"supervisor_messages": [HumanMessage(content=f"{research_brief}.")]}, config=thread)
